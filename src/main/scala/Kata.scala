...
def ??? = throw new Exception("not implemented yet") 
// Exercise 1
...
def add(x: Natural, y: Natural): Natural = y match {
  case Zero => x
  case Succ(pred) => add(Succ(x), pred)
}
...
// Elegance: 0.5 marks
// Total: 4
def sum(is: List[Natural]): Natural = List.foldLeft(is, Zero, add)
...
// Elegance: 0.5 marks
// Total: 4
def length[A](as: List[A]): Natural = as match {
  case Empty => Zero
  case Cons(_, tail) => Succ(length(tail))
}
}
